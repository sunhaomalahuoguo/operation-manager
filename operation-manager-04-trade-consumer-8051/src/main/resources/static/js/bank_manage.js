...
// *******************************************
  ...
  function bank_do_back(){
    var action  = window.document.forms[0].action;
    action += '?method=query';
    window.document.forms[0].action = action;
  }

  function bank_detail_fix(){
    var action  = window.document.forms[0].action;
    action += '?method=query';
    window.document.forms[0].action = action;
    window.document.forms[0].submit();
  ...
  function bank_detail_do_modify(){
    var action  = window.document.forms[0].action;
    action += '?method=modify';
    window.document.forms[0].action = action;
  }

  function bank_submit_form(methodName) {
    window.document.forms[0].action = "bankManageAction.do?method=" + methodName;
    window.document.forms[0].submit();
  }

  function bank_change_page(step) {
    ...
    bank_submit_form('query');
  }

  function bank_submit_by_enterkey() {
    if(13 == window.event.keyCode){
      bank_change_page(0);
      return false;
    }
  }
  function bank_delete(hid_bankid) {
  	if(!window.confirm('请确认该银行相关的交易数据已对账清算，确定删除银行及其绑定协议？')){return;}
    ...
    window.document.getElementById('selectedId').value = bankinfo_id;
		bank_submit_form('remove');
  }
	function bank_delete_bankinfo(hid_bankid) {
    if(hid_bankid) {
      if(!window.confirm('请确认所有选中银行相关的交易数据已对账清算，确定删除银行及其绑定协议？')){
        return;
      }
    } else {
  ...
  function bank_modify_bankinfo(hid_bankid) {
    var bankinfo_id = window.document.getElementById(hid_bankid).value;
    window.document.getElementById('selectedId').value = bankinfo_id;
	bank_submit_form('modify');
  }

  function bank_view_bankinfo(hid_bankid) {
    var bankinfo_id = window.document.getElementById(hid_bankid).value;
    window.document.getElementById('selectedId').value = bankinfo_id;
	bank_submit_form('view');
  }

  function bank_switch_checkbox(status) {
    var checkboxs = window.document.getElementsByName('bankInfoChk');
    for(var i = 0; i < checkboxs.length; i++) {
