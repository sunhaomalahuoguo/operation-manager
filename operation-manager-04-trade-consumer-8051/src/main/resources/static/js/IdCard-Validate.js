...
function last18(id17){
	...
	if (17==id17.length) {
        ...
        for(var i = 0; i < id17.length; i++){  
            // 对前17位数字与权值乘积求和
            sum += parseInt(id17.substr(i, 1), 10) * arrExp[i];  
        }  
        // 计算模（固定算法）
        idx = sum % 11;
        return arrValid[idx];
...
function isCnNewID(cid){
	return (cid.length==18&&(cid.substr(cid.length-1, 1).toUpperCase() == last18(cid.substr(0, cid.length-1))));
} 
